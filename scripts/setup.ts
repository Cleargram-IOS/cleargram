...
async function ensureBranch(commit: string) {
  ...
  const existed = await hasLocalBranch(worktreeDir, workBranch)

  if (existed) {
    step(`Checking out ${workBranch}`)
    await repo`git checkout ${workBranch}`
  } else {
    step(`Creating local work branch at ${commit}`)
    await repo`git checkout -B ${workBranch} ${commit}`
  }
...
async function ensureStgitStack(commit: string, branchExisted: boolean, force: boolean) {
  ...
  if (await hasStgitStack(worktreeDir, workBranch)) {
  ...
  if (branchExisted) {
    ...
    if (head !== commit) {
      if (!force) {
        throw new Error(
          `Branch ${workBranch} exists without StGit metadata and is not at ${commit} (re-run with --force to reset it)`,
        )
      }
      await assertCleanWorktree(`reset ${workBranch}`)
      step(`Resetting ${workBranch} to ${commit}`)
